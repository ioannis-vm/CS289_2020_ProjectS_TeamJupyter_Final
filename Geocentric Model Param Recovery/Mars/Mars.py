...
class ParameterReoveryModel(nn.Module):
    ...
    def forward(self, t):
        ...
        theta = torch.atan2((torch.tan(self.Omega * t + self.phi)),
                            (1-(2*self.k /
                                (self.k*torch.cos(self.Omega*self.k)
                                 - sqrt_term))))
...
def plot_prediction(i_start, i_end, plt_density):
    ...
    ax[0].plot(x_train, y_train[:, 0], '--', color='grey',
               label='Training Data')
    ax[0].plot(x_train, y_pred[:, 0], color='black', label='Prediction')
    ax[0].legend()
    ...
    ax[1].plot(x_train, y_train_norm, '--', color='grey',
               label='Training Data')
    ax[1].plot(x_train, y_pred_norm, color='black', label='Prediction')
    ax[1].legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    # Load the dataset
    x_all, y_all = load_dataset('../Geo_Data', 'Mars.csv')

    # from jdcal import gcal2jd
    # yr = np.sum(gcal2jd(100,1,1))
    # x_all[1132251] - yr
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - 10 year of data
    x_train, y_train = sample_data(x_all, y_all, 1132251, 365*10+1132251, 1)
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - 1000 year of data
    x_train, y_train = sample_data(x_all, y_all, 1132251, 365*10+1132251, 400)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Create a dataset and a dataloader
    ...
    # Plot the predictions
    plot_prediction(1132251, 365*10+1132251, 400)
    # Save the model
    torch.save(model.state_dict(), PATH)
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - 1000 year of data
    x_train, y_train = sample_data(x_all, y_all, 1132251, 365*5+1132251, 400)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Create a dataset and a dataloader
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - 10 year of data
    x_train, y_train = sample_data(x_all, y_all, 1132251, 365*5+1132251, 1)
    # (x_train[-1] - x_train[0])/365.22
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Create a dataset and a dataloader
    ...
    # Plot the predictions
    plot_prediction(1132251-365*5, 365*10+1132251, 400)
    # Save the modeL
    torch.save(model.state_dict(), PATH)
    ...
    model = ParameterReoveryModel().to(device)
    model.load_state_dict(torch.load(PATH))
    plot_residual(x_train_tensor, y_train_tensor, 50)
    # Resample - 200 years of data
    x_train, y_train = sample_data(x_all, y_all, 1132251-365*50,
                                   365*50+1132251, 1)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    plot_residual(x_train_tensor, y_train_tensor, 200)
    #######################################
