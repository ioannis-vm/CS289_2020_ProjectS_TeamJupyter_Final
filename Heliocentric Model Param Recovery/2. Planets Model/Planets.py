...
def load_dataset(rel_path, file_name):
    ...
    # y += np.random.normal(loc=0.00, scale=0.05, size=np.shape(y))
    return x, y
...
def print_parameter_values(fname, file=False):
    x = []
    for name, param in model.named_parameters():
        x.append([name, "{:e}".format(param.item())])
    print(pd.DataFrame(x))
    if file:
        pd.DataFrame(x).to_csv(fname)
    x = []
...
def brute2(param, rrange, current_loss, figure=True):
    vals = rrange
    losses = np.empty((len(vals)))
    for i in tqdm(range(len(vals))):
        val = vals[i]
        with torch.no_grad():
            setattr(model, param, nn.Parameter(torch.tensor([val],
                    requires_grad=True, dtype=torch.float64).to(device)))
        yhat = model(x_train_tensor)
        er1 = torch.abs(yhat-y_train_tensor)
        er2 = (2*np.pi) - er1
    ...
    minimizer = vals[np.argmin(losses)]
    with torch.no_grad():
        setattr(model, param, nn.Parameter(torch.tensor([minimizer],
                requires_grad=True, dtype=torch.float64).to(device)))
    if figure:
        plt.figure()
...
cel_bodies = ['Jupiter', 'Mars', 'Mercury', 'Neptune', 'Saturn', 'Venus']

if __name__ == '__main__':
    cel_body = 'Saturn'
    x_all, y_all = load_dataset('../Heliocentric_Data', cel_body+'.csv')
    # Set the seed for reproducibility
    # torch.manual_seed(42)  # No random numbers so far
    # Load parameters for the Earth's orbit
    params = load_earth_params('../Heliocentric_Data/', 'earth')
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - XXX years of data
    x_train, y_train = sample_data(x_all, y_all, 0, 365*1, 1)
    ...
    # Include 100 years of data
    x_train, y_train = sample_data(x_all, y_all, 0, 365*100, 10)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Define grid-search points
    dmrange = np.linspace(1.00E-4, 1.00E-3, 1000)
    ...
    x_train, y_train = sample_data(x_all, y_all, 0, 365*10, 10)
    # Create a dataset and a dataloader
    train_data = TensorDataset(x_train_tensor, y_train_tensor)
    train_loader = DataLoader(dataset=train_data,
    ...
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Define grid-search points
    dmrange = np.linspace(minimizer*0.5, minimizer*1.8, 1000)
    minimizer, new_loss = brute(dmrange, losses[-1])
    # refine, repeat
    dmrange = np.linspace(minimizer*0.95, minimizer*1.05, 1000)
    minimizer, new_loss = brute(dmrange, losses[-1])
    # Save the model
    torch.save(model.state_dict(), PATH)
    ...
    print()
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    print('Phase 4 - Train on 100 years of data ')
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - 200 year of data
    x_train, y_train = sample_data(x_all, y_all, 0, 365*100, 100)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Create a dataset and a dataloader
    ...
    # Train the model
    losses = train_model(grad_weights, 1.0E-2, 2000)
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - 200 year of data
    x_train, y_train = sample_data(x_all, y_all, 0, 365*200, 100)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Create a dataset and a dataloader
    ...
    # Train the model
    losses = train_model(grad_weights, 1.0E-5, 1000)
    ...
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Resample - all
    x_train, y_train = sample_data(x_all, y_all, 0, len(x_all), 200)
    # Convert training data to tensors
    x_train_tensor = torch.from_numpy(x_train).to(device)
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Create a dataset and a dataloader
    ...
    # Train the model
    losses = train_model(grad_weights, 1.0E-7, 10000, tol=1E-10)
    ...
    print()
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    print('Check - Verify global minimum w.r.t. XX')
    print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
    # Load the model
    model = ParameterReoveryModel(params).to(device)
    model.load_state_dict(torch.load(PATH))
    ...
    y_train_tensor = torch.from_numpy(y_train).to(device)
    # Define grid-search points
    rrange = np.linspace(0.1E1, 3E1, 1000)
    lsses = brute2('a0', rrange, 0.00)  # Specify parameter here
    # Load the model (restore)
    model = ParameterReoveryModel(params).to(device)
    model.load_state_dict(torch.load(PATH))
    print_parameter_values()
    ...
    model = ParameterReoveryModel(params).to(device)
    model.load_state_dict(torch.load(PATH))
    plot_residual(x_train_tensor, y_train_tensor, 205)
    # Load the model (restore)
    model = ParameterReoveryModel(params).to(device)
    model.load_state_dict(torch.load(PATH))
    ...
    model.load_state_dict(torch.load(PATH))
    # Output parameters
    print_parameter_values(cel_body, file=True)
